...
TradesList.prototype = {
    showTrades: function (req, res) {
        var tradesList = [];
        ...
        connection.on('connect', (err) => {
            ...
            console.log(req);
            //res.render('index')
            var request = new Request("SELECT DISTINCT vessel_id, origin_date, site_origin, seller, site_destination, destination_date, buyer, charterer, value, product_id, id FROM dbo.app_view_trades", (err, rowsLength, rows) => {
                if (err) {
                    console.log(err);
                } else {
    ...
    insertTrade: function (req, res, next) {
        ...
        connection.on('connect', (err) => {
            ...
            var request = new Request("EXEC insertTrade @vessel_id, @origin_date, @origin_id, @seller, @charterer, @buyer, @destination_id, @destination_date, @reference_id, @updated_by, @product_id, @value, @unit_of_measure_id", (err) => {
            ...
            request.addParameter('origin_date', TYPES.Date, new Date(req.body.origin_date)); 
            ...
            request.addParameter('destination_date', TYPES.Date, new Date(req.body.origin_date)); 
    ...
    deleteTrade: function (req, res) {
        ...
        connection.on('connect', (err) => {
            ...
            var request = new Request("DELETE FROM [dbo].[trades] WHERE id = @trade_id;", (err) => {
                if (err) {
                    console.log(err);
                } else {
                    return req.body;
    ...
    updateTrade: function (req, res) {
        ...
        connection.on('connect', (err) => {
            ...
            var request = new Request("UPDATE [dbo].[trades] SET [seller] = @seller, buyer = @buyer, charterer = @charterer, origin_date = @origin_date WHERE id = @trade_id;", (err) => {
                if (err) {
                    console.log(err);
                } else {
                    return req.body;
